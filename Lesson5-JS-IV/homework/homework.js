...
function sumArray(numbers, cb) {
 ...
 var result = numbers.reduce(function(accumulator, number){ 
...
function forEach(arr, cb) {
 ...
 arr.forEach(function(sacredWord){
   cb(sacredWord);
 });
}
...
function map(arr, cb) {
  // create a new array
  // iterate over each value in arr, pass it to cb, then place the value returned from cb into the new array
  // the new array should be the same length as the array argument
  // return the new array
  let topHat = [];
  function aMagicTrick(rabbitsAndStuff){
   topHat.push(cb(rabbitsAndStuff));
  }
  arr.map(aMagicTrick);
  return topHat;
}
